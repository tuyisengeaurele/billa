import { beforeAll, beforeEach, describe, expect, it } from "vitest";
import request from "supertest";
import { createApp } from "../app.js";
import { prisma } from "../lib/prisma.js";
import { resetDb } from "../test/db.js";

beforeAll(() => {
  process.env.JWT_ACCESS_SECRET ??= "test-secret";
  process.env.JWT_REFRESH_TTL ??= "30d";
});

beforeEach(resetDb);

async function registerAndGetCookies(app: ReturnType<typeof createApp>, email: string, businessName: string) {
  const res = await request(app).post("/auth/session").send({
    idToken: JSON.stringify({ uid: email, email }),
    businessName,
  });
  return { cookies: res.headers["set-cookie"] as unknown as string[], userId: res.body.user.id as string };
}

async function setUpMember(app: ReturnType<typeof createApp>) {
  const { cookies: ownerCookies } = await registerAndGetCookies(app, "owner@example.com", "Kigali Traders");
  const ownerRes = await request(app).get("/auth/me").set("Cookie", ownerCookies);
  const businessId = ownerRes.body.business.id as string;
  const { cookies: memberOwnCookies, userId: memberId } = await registerAndGetCookies(
    app,
    "member@example.com",
    "Member's Own Biz",
  );
  await prisma.businessMember.create({ data: { businessId, userId: memberId } });

  const switchRes = await request(app)
    .post("/auth/switch-business")
    .set("Cookie", memberOwnCookies)
    .send({ businessId });
  return { memberCookies: switchRes.headers["set-cookie"] as unknown as string[], businessId };
}

describe("business settings routes reject members", () => {
  it("blocks PATCH /business for a member", async () => {
    const app = createApp();
    const { memberCookies } = await setUpMember(app);

    const res = await request(app).patch("/business").set("Cookie", memberCookies).send({ name: "Renamed" });

    expect(res.status).toBe(403);
    expect(res.body.error).toBe("not_owner");
  });

  it("blocks PUT /business/sequences for a member", async () => {
    const app = createApp();
    const { memberCookies } = await setUpMember(app);

    const res = await request(app)
      .put("/business/sequences")
      .set("Cookie", memberCookies)
      .send([{ type: "INVOICE", prefix: "INV-", nextNumber: 1 }]);

    expect(res.status).toBe(403);
  });

  it("allows GET /business for a member", async () => {
    const app = createApp();
    const { memberCookies } = await setUpMember(app);

    const res = await request(app).get("/business").set("Cookie", memberCookies);

    expect(res.status).toBe(200);
    expect(res.body.business.name).toBe("Kigali Traders");
  });
});
