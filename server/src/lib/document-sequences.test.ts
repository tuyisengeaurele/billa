...
describe("mergeSequences", () => {
  ...
  it("uses the saved row for a type that has one, defaults for the rest", () => {
    const result = mergeSequences([{ type: "INVOICE", prefix: "CUSTOM-", nextNumber: 42, resetYearly: true }]);
    const invoice = result.find((r) => r.type === "INVOICE");
    const quote = result.find((r) => r.type === "QUOTE");
    expect(invoice).toEqual({ type: "INVOICE", prefix: "CUSTOM-", nextNumber: 42, resetYearly: true });
    expect(quote).toEqual({ type: "QUOTE", prefix: "QTE-", nextNumber: 1, resetYearly: false });
  });
});
