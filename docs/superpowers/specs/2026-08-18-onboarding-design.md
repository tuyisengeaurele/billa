...
`Dashboard.tsx` is a new minimal stub page (business name plus a welcome
line); the real dashboard/documents UI is a separate future stage.
...
union of `z.literal("")` and a proper email check; format only matters once
something's actually typed. The server's `PATCH /business` contract
...
1. **upload**: dropzone/file input, "Upload your logo" prompt, skip link.
   On file select: `POST /business/logo` (multipart) returns `url`.
...
4. **Confirm**: `POST /business/logo/confirm` with `url` + `primaryColor` +
   `accentColors`, then navigate to `/dashboard`.
...
- `LogoStep`: upload to processing to review transitions, confirm posts the
...
- `Onboarding`: renders the right step component, progress indicator text,
  step transition from details to logo.
