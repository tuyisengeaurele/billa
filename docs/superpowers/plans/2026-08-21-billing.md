...
Run: `DATABASE_URL="postgresql://postgres:YOUR_LOCAL_DB_PASSWORD@localhost:5432/billa_test" npx prisma migrate reset --force`

Generate the migration SQL manually (non-interactive `migrate dev` isn't supported in this environment):

Run: `cd server && npx prisma migrate diff --from-url "postgresql://postgres:YOUR_LOCAL_DB_PASSWORD@localhost:5432/billa" --to-schema-datamodel prisma/schema.prisma --script`
...
Run: `DATABASE_URL="postgresql://postgres:YOUR_LOCAL_DB_PASSWORD@localhost:5432/billa_test" npx prisma migrate deploy`
